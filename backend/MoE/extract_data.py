...
def global_info_text_v2(indf):
    '''
    indf: all_info_table
    used keys: ['data_source','select_celltype','layer','has_recon_axon','has_recon_den','has_apical','has_local','hemisphere']
    '''
...
def fea2text_v2(indf, feadf, axonfea=True, detail_types=2,
    ...
    for s in ds.index:
        ...
        sdf_types = indf['select_celltype'].value_counts()
        for i, t in enumerate(sdf_types.index):
            if i >= detail_types:
                break
            ndf = sdf[(sdf.select_celltype == t)].index
            if not axonfea:
                ndf = sdf[(sdf.select_celltype == t) & (sdf.has_recon_den == 1)].index
            if len(ndf) <= 1:
                continue
            # print(s)
            if axonfea:
            ...
            for f, fea in enumerate(usedfeas):
                fmean = feadf.loc[ndf, fea].mean()
                fmean = round(fmean, 2)
                fstd = feadf.loc[ndf, fea].std()
                fstd = round(fstd, 2)
                if f < 2:
